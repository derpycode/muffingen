function [k1out] = make_grid_runoff_rnd(k1in,str,opt_debug)
...
%   17/02/17: adapted for muffingen framework
%            -> no reading in of k1 file (but still need filename/string)
%            -> return adjusted and expanded k1 file
%   17/03/21: added output option
%
%   ***********************************************************************
    ...
    if k1_set(j,i) == 0
        ...
        while k1_set(j,i) == 0
            ...
            if min([k1_try]) < 90
                ...
                while k1_set(j,i) == 0
                    [j_probe,i_probe,dir_probe,j_dir,i_dir] = fun_ijprobe(j,i);
...
% NOTE: mark first row: 88, 89
% (so, slightly from standard/original GENIE format)
% k1_new(1,:) = 18;
% k1_new(j_max,:) = 94;
k1_new(1,:) = 88;
k1_new(j_max,:) = 89;
